...
class AudioEngine:
    def __init__(self, audio_profile=None, use_kokoro=True, use_gemini=True):
        from modules.audio.audio_profiles import AudioProfile
        self.profile = audio_profile or AudioProfile()

        self.output_dir = os.path.join(os.getcwd(), "assets", "audio_clips")
        os.makedirs(self.output_dir, exist_ok=True)

        self.min_scene_duration = self.profile.min_scene_duration
    ...
    def stylize_for_gemini(self, text):
        ...
        return f"{self.profile.gemini_style_prompt} [short pause] {cleaned}"
    ...
    def _try_gemini(self, text, output_path_wav):
        ...
        url = (
            ...
            f"{self.profile.gemini_model}:generateContent?key={self.gemini_api_key}"
        )
    ...
    def _try_kokoro(self, text, output_path_wav):
        ...
        try:
            generator = self._kokoro_pipeline(
                ...
                voice=self.profile.kokoro_french_voice
            )
    ...
    async def _try_edge(self, text, output_path_mp3):
        ...
        communicate = edge_tts.Communicate(
            ...
            voice=self.profile.edge_fallback_voice,
            rate=self.profile.edge_fallback_rate,
            volume=self.profile.edge_fallback_volume,
            pitch=self.profile.edge_fallback_pitch,
        )
