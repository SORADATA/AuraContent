...
class ArchiveProvider:
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                for item in r.json().get("results", []):
                    img_id = item.get("id")
                    if is_used(self.history, "openverse", img_id): continue
    ...
    def _wiki_api(self, params):
        ...
        for _ in range(2):
            try:
                r = requests.get(url, params=params, headers=self.wiki_headers, timeout=10)
                if r.status_code == 200: return r
                if r.status_code == 429: time.sleep(int(r.headers.get("Retry-After", 5)))
            except:
                pass
        return None
    ...
    def _process_wiki_pages(self, pages, output_path, query, min_relevance):
        for page_id, info in pages.items():
            ...
            if not img_infos: continue
            
            img_url = img_infos[0].get("url", "")
            title = info.get("title", "")
            
            if not img_url or "pdf" in img_url.lower(): continue
            if is_used(self.history, "wikimedia", page_id): continue
    ...
    def get_wikimedia(self, query, output_path):
        ...
        if self.get_openverse(query, output_path): return True
        time.sleep(0.5)
