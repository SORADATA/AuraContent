...
def chunk_words_for_subtitles(words: list[str], max_words: int = 3) -> list[list[str]]:
    chunks = []
    current = []
...
def generate_grouped_srt(text: str, duration: float, output_path: str, max_words_per_caption: int = 3, min_caption_dur: float = 0.45) -> str | None:
    words = text.split()
    if not words:
        return None
